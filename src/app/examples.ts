...
@Component({
    ...
    template: `
        <div class='parent'>
            <label for='hy'>There are plenty of events along the lifetime of a drag event. <a
            ...
            <div class='wrapper'>
                <div class='container' [dragula]='"second-bag"'>
                    ...
                    <div>The <code>shadow</code> event fires whenever the placeholder showing where an element would be
                        dropped is moved to a different container or position
                    </div>
                    <div>A <code>drop</code> event is fired whenever an element is dropped anywhere other than its
                        origin <em>(where it was initially dragged from)</em></div>
                </div>
                <div class='container' [dragula]='"second-bag"'>
                    ...
                    <div>A <code>cancel</code> event is fired when an element would be dropped onto an invalid target,
                        but retains its original placement instead
                    </div>
...
@Component({
    ...
    template: `
        <div class='parent'>
            <label for='hy'>By default, dropping an element outside of any known containers will keep the element in the
                last place it went over. You can make elements go back to origin if they're dropped outside of known
            ...
            <div class='wrapper'>
                <div class='container' [dragula]='"sixth-bag"'>
                    ...
                    <div><span class='handle'>+</span>Note that <code>handle</code> element in the <code>moves</code>
                        handler is just the original event target.
                    </div>
                </div>
                <div class='container' [dragula]='"sixth-bag"'>
      ...
      moves: function (el, container, handle) {
        return handle.className === 'handle';
            ...
            <div>There are a few similar mechanisms to determine whether an element can be dragged from a certain
                container <a href='https://github.com/bevacqua/dragula#optionsmoves'>(<code>moves</code>)</a>, whether
                an element can be dropped into a certain container at a certain position <a
                        href='https://github.com/bevacqua/dragula#optionsaccepts'>(<code>accepts</code>)</a>, and
                whether an element is able to originate a drag event <a
...
@Component({
    ...
    template: `
        <div class='parent'>
            <label><strong>Click or Drag!</strong> Fires a click when the mouse button is released before a <code>mousemove</code>
                event, otherwise a drag event is fired. No extra configuration is necessary.</label>
            <div class='wrapper'>
                <div class='container' [dragula]='"seventh-bag"'>
                    <div (click)='onclick("one")'>
                        {{clicked.one ? "Clicked!" : "Clicking on these elements triggers a regular click event you can listen to."}}
                    </div>
                    <div (click)='onclick("two")'>
                        {{clicked.two ? "Clicked!" : "Try dragging or clicking on this element."}}
                    </div>
                    <div (click)='onclick("three")'>
                        {{clicked.three ? "Clicked!" : "Note how you can click normally?"}}
                    </div>
                    <div (click)='onclick("four")'>{{clicked.four ? "Clicked!" : "Drags don't trigger click events."}}
                    </div>
                    <div (click)='onclick("five")'>
                        {{clicked.five ? "Clicked!" : "Clicks don't end up in a drag, either."}}
                    </div>
                    <div (click)='onclick("six")'>
                        {{clicked.six ? "Clicked!" : "This is useful if you have elements that can be both clicked or dragged."}}
                    </div>
                    <div (click)='onclick("seven")'>
...
@Component({
    ...
    template: `
        <div class='parent'>
            <label for='hy'>Tabling!</label>
            <table>
                <thead>
                <tr class="container" [dragula]="'table-bag'" [dragulaModel]="listy">
                    <th class="dragula-ignored">dragula-ignored</th>
                    <th *ngFor="let item of listy">{{item}}
                        <button class="drag-handle">o</button>
                    </th>
                    <th class="dragula-ignored"></th>
                </tr>
                </thead>
                <tbody>
                <tr>
...
export const EXAMPLES: any[] = [ExampleA, ExampleB, AnotherExample, SuchExample, VeryExample, MuchExample, WowExample,
    RepeatExample, NestedRepeatExample, TableExample];
